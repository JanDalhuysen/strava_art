...
// -----------------------------------------------------------------------------
struct Point { double x, y; };   // image coords
std::vector<Point> skeleton_to_points(const char *fname)
{
    ...
    img.dilate(2); img.erode(2);    // remove noise
    img.skeleton();                 // 1-pixel wide centerline

    std::vector<Point> pts;
    cimg_forXY(img, x, y)
        if (img(x, y)) pts.push_back({(double)x, (double)y});
    return pts;
}
...
// -----------------------------------------------------------------------------
std::vector<fmm::Point> geo_place(const std::vector<Point>& pts,
                                  double clat, double clon,
                                  double km_width = 10.0)
{
...
MatchResult match_trajectory(const std::vector<fmm::Point>& pts,
...
// -----------------------------------------------------------------------------
void search_best(const std::vector<fmm::Point>& art,
                 const std::string& ubodt,
                 const std::string& net,
                 double center_lat, double center_lon,
                 double step_km = 2.0, int grid = 5)
{
    std::mt19937 rng(42);
    ...
    for (int dx = -grid; dx <= grid; ++dx)
        for (int dy = -grid; dy <= grid; ++dy)
        {
            ...
            double try_lon = center_lon + dx * step_km /
                             (111.32 * std::cos(center_lat * M_PI / 180.0));
